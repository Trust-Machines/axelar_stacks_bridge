...
describe('gas-service contract test suite', () => {
    ...
    it('ensures initial state is correct', async () => {
        const balance = await simnet.callReadOnlyFn('gas-service', 'get-balance', [], deployer);
        const owner = await simnet.callReadOnlyFn('gas-service', 'get-contract-owner', [], deployer);
        const isDeployerOwner = await simnet.callReadOnlyFn('gas-service', 'is-contract-owner', [], deployer);
        const isWallet1Owner = await simnet.callReadOnlyFn('gas-service', 'is-contract-owner', [], wallet1);
    ...
    it('allows refunding (topping up) the contract', async () => {
        ...
        const refundTx = await simnet.callPublicFn('gas-service', 'refund', [Cl.uint(refundAmount), Cl.principal(wallet1)], wallet1);
    ...
    it('prevents refunding with invalid amount', async () => {
        const refundTx = await simnet.callPublicFn('gas-service', 'refund', [Cl.uint(0), Cl.principal(wallet1)], wallet1);
    ...
    it('allows owner to pay native gas for contract call', async () => {
        ...
        await simnet.callPublicFn('gas-service', 'refund', [Cl.uint(2000), Cl.principal(wallet1)], wallet1);

        const payTx = await simnet.callPublicFn('gas-service', 'pay-native-gas-for-contract-call', [
            Cl.uint(1000),
    ...
    it('allows owner to add native gas', async () => {
        ...
        const addGasTx = await simnet.callPublicFn('gas-service', 'add-native-gas', [
            Cl.uint(500),
            Cl.principal(wallet2),
    ...
    it('prevents paying or adding gas more than contract balance', async () => {
        // First, refund the contract to have some balance
        await simnet.callPublicFn('gas-service', 'refund', [Cl.uint(1000), Cl.principal(wallet1)], wallet1);
    ...
    it('allows owner to transfer ownership', async () => {
        const transferTx = await simnet.callPublicFn('gas-service', 'transfer-ownership', [Cl.principal(wallet1)], deployer);
        expect(transferTx.result).toBeOk(Cl.bool(true));

        const newOwner = await simnet.callReadOnlyFn('gas-service', 'get-contract-owner', [], deployer);
        expect(newOwner.result).toBeOk(Cl.principal(wallet1));
    });

    it('prevents non-owner from transferring ownership', async () => {
        const transferTx = await simnet.callPublicFn('gas-service', 'transfer-ownership', [Cl.principal(wallet2)], wallet1);
