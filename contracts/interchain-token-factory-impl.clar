...
(define-read-only (get-interchain-token-deploy-salt
        (deployer principal)
        (salt (buff 32))
    )
    (keccak256 (concat (concat PREFIX-INTERCHAIN-TOKEN-SALT CHAIN-NAME-HASH)
        (concat (unwrap-panic (to-consensus-buff? deployer)) salt)
    ))
)
...
(define-read-only (get-canonical-interchain-token-deploy-salt (token-address principal))
    (keccak256 (concat (concat PREFIX-CANONICAL-TOKEN-SALT CHAIN-NAME-HASH)
...
(define-private (get-interchain-token-id-raw
        (its-impl <its-trait>)
        (salt (buff 32))
    )
    (contract-call? .interchain-token-service interchain-token-id its-impl
        TOKEN-FACTORY-DEPLOYER salt
    )
)
...
;; @param salt A unique identifier used in the deployment process.
;; @return (buff 32) The ID of the interchain token.
(define-public (get-interchain-token-id
        (its-impl <its-trait>)
        (deployer principal)
        (salt (buff 32))
    )
...
(define-public (get-canonical-interchain-token-id
        (its-impl <its-trait>)
        (token-address principal)
    )
    ;; this is assumed to be a read only operation
    ;; #[allow(unchecked_data)]
    (get-interchain-token-id-raw its-impl
        (get-canonical-interchain-token-deploy-salt token-address)
    )
)
...
(define-public (register-canonical-interchain-token
    ...
    (begin
        ...
        (contract-call? .interchain-token-service deploy-token-manager
            ...
            LOCAL-DEPLOYMENT TOKEN-TYPE-LOCK-UNLOCK
            (unwrap-panic (to-consensus-buff? {
                operator: none,
                token-address: (contract-of token),
            }))
            token-manager verification-params
        )
    )
)
...
(define-public (deploy-remote-canonical-interchain-token
    ...
    (let (
            ...
            (token-id (unwrap!
                (get-canonical-interchain-token-id its-impl (contract-of token))
                ERR-SERVICE-NOT-DEPLOYED
            ))
            ;; This ensures that the token manager has been deployed by this address, so it's safe to trust it.
            (minter NULL-BYTES)
        )
        (asserts! (is-proxy) ERR-NOT-PROXY)
        (try! (contract-call? .interchain-token-service valid-token-address its-impl
            token-id
        ))
        (deploy-remote-interchain-token-inner gateway-impl gas-service-impl
            its-impl deploy-salt minter destination-chain gas-value token
        )
    )
)
...
(define-public (deploy-remote-interchain-token
    ...
    (deploy-remote-interchain-token-with-minter gateway-impl gas-service-impl
        its-impl salt_ minter_ destination-chain none gas-value token
        token-manager caller
    )
)
...
(define-public (deploy-remote-interchain-token-with-minter
    ...
    (let (
            ...
            (minter (if (not (is-eq NULL-ADDRESS minter_))
                (begin
                    ;; #[filter(token-manager)]
                    (try! (check-token-minter token-manager deployed-token-id minter_))
                    (asserts! (not (is-eq minter_ (contract-of its-impl)))
                        ERR-INVALID-MINTER
                    )
                    (match destination-minter
                        destination-minter-unpacked (begin
                        ...
                        (unwrap!
                            (as-max-len?
                                (unwrap-panic (to-consensus-buff? minter_))
                                u128
                            )
                            ERR-INVALID-MINTER
                        )
...
(define-private (get-deploy-approval-key (approval {
    ...
    (keccak256 (concat PREFIX-DEPLOY-APPROVAL (unwrap-panic (to-consensus-buff? approval))))
)
...
(define-public (approve-deploy-remote-interchain-token
    ...
    (let (
            ...
            (token-info (unwrap!
                (contract-call? .interchain-token-service-storage get-token-info
                    token-id
                )
                ERR-TOKEN-NOT-FOUND
            ))
        ...
        (asserts! (is-eq (contract-of token) (get manager-address token-info))
            ERR-TOKEN-MISMATCH
        )
        ;; #[filter(minter)]
        (try! (check-token-minter token token-id minter))
        (asserts!
            (contract-call? .interchain-token-service-storage is-trusted-chain
                destination-chain
            )
            ERR-INVALID-CHAIN-NAME
        )
        (try! (contract-call? .interchain-token-service-storage
            emit-deploy-remote-interchain-token-approval minter deployer
            token-id destination-chain destination-minter
        ))
        (contract-call? .interchain-token-service-storage
            set-approved-destination-minter approval-key
            (keccak256 destination-minter)
        )
    )
)
...
(define-public (revoke-deploy-remote-interchain-token
    ...
    (let (
            ...
            (is-deleted (unwrap!
                (contract-call? .interchain-token-service-storage
                    remove-approved-destination-minter key
                )
                ERR-CANNOT-DELETE-DEPLOY-APPROVAL
            ))
        )
        (try! (require-not-paused))
        (asserts! is-deleted ERR-REMOTE-DEPLOYMENT-NOT-APPROVED)
        (try! (contract-call? .interchain-token-service-storage
            emit-revoked-deploy-remote-interchain-token-approval minter
            deployer token-id destination-chain
        ))
        (ok true)
    )
)
...
(define-private (use-deploy-approval
    ...
    (let (
            ...
            (approved-minter-hash (default-to NULL-BYTES
                (contract-call? .interchain-token-service-storage
                    get-approved-destination-minter key
                )))
        )
        (asserts! (is-eq approved-minter-hash destination-minter-hash)
...
(define-private (check-token-minter
    ...
    (let (
        ...
        (asserts!
            (unwrap! (contract-call? token-manager is-minter minter)
                ERR-TOKEN-NOT-DEPLOYED
            )
            ERR-NOT-MINTER
        )
        (asserts! (not (is-eq current-its-impl minter)) ERR-INVALID-MINTER)
        (asserts! (not (is-eq .interchain-token-service minter))
            ERR-INVALID-MINTER
        )
        (ok true)
    )
...
(define-private (deploy-remote-interchain-token-inner;; #[allow(unchecked_params)]
        (gateway-impl <gateway-trait>)
        (gas-service-impl <gas-service-trait>)
        (its-impl <its-trait>)
        (deploy-salt (buff 32))
    ...
    (let (
        ...
        (contract-call? .interchain-token-service deploy-remote-interchain-token
            gateway-impl gas-service-impl its-impl deploy-salt
            destination-chain name symbol decimals minter gas-value
        )
    )
)
