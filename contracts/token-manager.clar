...
(impl-trait 'ST1DRWEN0XDBPSEX3XVXK57NNT03B6ZZ0D58N36RS.traits.token-manager-trait)
...
(define-constant ERR-FLOW-LIMITERS-FULL (err u170010))
...
(define-public (take-token
    ...
    (begin
        ...
        (transfer-token-from sip-010-token from (as-contract contract-caller)
            amount
        )
    )
)

(define-private (transfer-token-from
        (sip-010-token <sip-010-trait>)
        (from principal)
        (to principal)
        (amount uint)
    ...
    (begin
        (asserts! (var-get is-started) ERR-NOT-STARTED)
        (asserts!
            (is-eq (contract-of sip-010-token)
                (unwrap! (var-get token-address) ERR-NOT-STARTED)
            )
