...
(define-private (is-service-proxy)
    (is-eq contract-caller SERVICE-PROXY)
)
(define-private (is-service-impl)
    (is-eq contract-caller (var-get service-impl))
)

(define-private (is-proxy-or-service-impl)
    (or (is-service-proxy) (is-service-impl))
)
...
(define-read-only (get-service-impl)
    (var-get service-impl)
)
(define-read-only (get-service-proxy)
    SERVICE-PROXY
)
...
(define-read-only (get-factory-impl)
    (var-get factory-impl)
)
(define-read-only (get-factory-proxy)
    FACTORY-PROXY
)

(define-private (is-factory-proxy)
    (is-eq contract-caller FACTORY-PROXY)
)
(define-private (is-factory-impl)
    (is-eq contract-caller (var-get factory-impl))
)
(define-private (is-proxy-or-factory-impl)
    (or (is-factory-proxy) (is-factory-impl))
)
...
(define-map trusted-chain-address
    (string-ascii 20)
    (string-ascii 128)
)
...
(define-private (extract-and-set-trusted-address (entry {
    chain-name: (string-ascii 20),
    address: (string-ascii 128),
}))
...
(define-public (set-trusted-addresses (trusted-chain-names-addresses (list 50 {
    chain-name: (string-ascii 20),
    address: (string-ascii 128),
})))
    (begin
...
(define-map used-token-managers
    principal
    bool
)
...
;; logic for write guards will be in the calling context (proxy, impl)
;; #[allow(unchecked_data)]
(define-public (insert-token-manager
        (token-id (buff 32))
        (manager-address principal)
        (token-type uint)
    )
...
(define-map approved-destination-minters
    (buff 32)
    (buff 32)
)
...
;; logic for write guards will be in the calling context (proxy, impl)
;; #[allow(unchecked_data)]
(define-public (set-approved-destination-minter
        (approval-key (buff 32))
        (hashed-destination-minter (buff 32))
    )
    (begin
        (asserts! (is-proxy-or-factory-impl) ERR-NOT-AUTHORIZED)
        (ok (map-set approved-destination-minters approval-key
            hashed-destination-minter
        ))
    )
)
...
(define-public (emit-trusted-address-removed (chain-name (string-ascii 20)))
    (begin
        (asserts! (is-service-impl) ERR-NOT-AUTHORIZED)
        (print {
