...
(define-public (execute-deploy-interchain-token
    ...
    (begin
        ...
        (let (
            ...
            (its-hub-chain (get-its-hub-chain))
        )
        (asserts! (not (is-eq wrapped-source-chain its-hub-chain)) ERR-UNTRUSTED-CHAIN)
        (asserts! (is-trusted-chain wrapped-source-chain) ERR-UNTRUSTED-CHAIN)
        (asserts! (is-eq source-chain its-hub-chain) ERR-UNTRUSTED-CHAIN)
